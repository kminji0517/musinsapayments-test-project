...
### 1. 계층형 아키텍처
Controller → Service → Repository 구조로 관심사를 분리했습니다.
...
### 6. 포인트 사용/취소 이력 추적
- `point_usage_detail` 테이블을 통해 특정 시점에 적립된 포인트가 어떤 주문에서 얼마나 사용되었는지 추적합니다.
- `point_usage_cancel_detail` : 취소 시 적립건별 취소 금액을 기록합니다.
  - `usage_id + point_key + seq` 복합키로 부분 취소가 여러 번 발생해도 이력 추적이 가능한 구조로 설계했습니다.
  - 취소 시 만료일이 긴 순서로 잔액을 복구하여 유효기간이 짧은 포인트의 활용 기회를 보존합니다.

### 7. 포인트 사용 취소 시 만료된 포인트 처리
사용 취소 시점에 이미 만료된 포인트는 잔액을 복구하지 않고, 해당 금액만큼 신규 적립 처리합니다.
신규 적립된 포인트 키는 응답의 `newPointKeys` 필드로 반환됩니다.

### 8. 부분 취소 상태 관리
운영 효율성과 데이터 정합성을 고려해 포인트 사용 취소 시 전체 취소 여부에 따라 상태 코드를 구분해 관리합니다.
- 전체 취소: `CANCELLED`
...
| totalAmount | Long | 총 사용 금액 |
| remainingCancelAmount | Long | 취소 가능 잔여 금액 |
| cancelledAmount | Long | 이번 요청에서 취소된 금액 |
| newPointKeys | List\<String\> | 취소 시점에 만료된 포인트를 대신하여 신규 적립된 포인트 키 목록 (없으면 빈 리스트) |
| usedAt | LocalDateTime | 사용일자 |
